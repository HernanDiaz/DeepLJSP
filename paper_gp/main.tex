...
\item An automatic configuration study (irace, 200 experiments) of the
  ...
  selection pressure), in instructive contrast with the companion neural
  policy, for which operating-point tuning yields none
...
including the hyperparameter analysis and the comparison with the neural
policy. Section~\ref{sec:conclusions} concludes.
...
multi-objective formulations. Closest to our setting, Karunakaran et
...
learning paradigms for the same construction task~\cite{Xu2025}. Our
...
$C_{\max} = \max_j C_{o_{jm}}$ is itself an interval; following the IJSP
literature~\cite{Lei2012,DiazICAE2023}, schedules are ranked
lexicographically by upper bound (worst case) first. All methods in this
paper---the GP rules, every baseline, and the companion DRL
policy---share one implementation of this decoder and evaluator, so
reported differences cannot stem from evaluator discrepancies.
...
instance is derived from its crisp counterpart by replacing every duration
$p$ with an integer interval $[p-d,\, p+d]$ symmetric around it, with
half-width bounded by $d \le 0.15\,p$ (maximum deviation $15\%$); machine
routes are unchanged. The midpoint of every interval thus equals the
original duration---the crisp instance is exactly the midpoint scenario of
its interval version---which makes the crisp Taillard bounds a meaningful
...
is ${\approx}13\%$. This is the same $\pm15\%$ generation scheme used
throughout our prior work on the
IJSP~\cite{DiazFEABC2023,DiazICAE2023,DiazTS2026}, which makes the
comparison against those published metaheuristics an exact one (identical
uncertainty model and benchmark).
...
policy, enabling the matched comparison of Section~\ref{sec:vs-rl}---and
TA15--TA20 serve as development set: they never contribute to fitness,
...
\caption{Best-of-$N$ curves at matched inference budgets: the evolved
rule with uniform $\varepsilon$-randomization versus the companion DRL
policy sampling from its learned distribution.}
\label{fig:bestofn}
\end{figure}
...
The empirical conclusions are four. (i)~Evolved rules dominate every
...
pressure; notably, this small gain contrasts with the companion DRL
policy, for which operating-fidelity tuning yielded none. (iv)~Against
that neural policy under matched budgets, the interpretable rule matches
or beats it single-shot, and cedes ground only when both methods are
granted large sampling budgets, where the learned sampling distribution
of the policy is more efficient than uniform randomization.

Future work follows three lines. First, a terminal ablation to quantify
...
ensembles of complementary rules and co-evolution of construction and
tie-breaking. Third, using evolved rules as cheap, high-quality
