...
produces \emph{more robust} ones. Table~\ref{tab:robustness} reports mean
$\bar\varepsilon$ ($\times10^3$) over the 70 instances: the GP rule is more
robust than both constructive baselines at the nominal width
($5.8$ versus $6.5$ for MOR and $6.3$ for G\&T-MWKR), and its advantage
\emph{widens} as uncertainty grows (at $+40\%$: $7.7$ versus $8.9$ and
$8.6$). Because the rule can read interval widths, it favours dispatching
decisions whose realized outcome is less sensitive to how the durations
materialize. Figure~\ref{fig:robustness} shows the per-instance
distributions.

\begin{table}[t]
\centering\small
\caption{Mean $\bar\varepsilon$ robustness ($\times10^3$, lower is better)
over the 70 instances, at nominal interval widths and widened by $+20\%$
and $+40\%$. Monte Carlo with $K{=}1000$ realizations per instance,
...
Method & $+0\%$ & $+20\%$ & $+40\%$ \\
\midrule
GP rule & \textbf{5.8} & \textbf{6.8} & \textbf{7.7} \\
G\&T-MWKR & 6.3 & 7.4 & 8.6 \\
MOR & 6.5 & 7.7 & 8.9 \\
\bottomrule
\end{tabular}
\end{table}
