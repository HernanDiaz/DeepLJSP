...
log_filename = f"jobshop_rl_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_filename),  # Handler para archivo
        logging.StreamHandler()             # Handler para consola (mantiene los logs en consola también)
    ]
)
logger = logging.getLogger("JobShopRL")

class TrainingLogger:
    """Logger para guardar métricas de entrenamiento en CSV"""
    ...
    def log_step(self, episode, current_makespan, best_makespan, avg_makespan, training_time):
        """Registra las métricas de un paso de entrenamiento"""
        self.metrics.append({
            'Episode': episode,
            'CurrentMakespan': current_makespan,
            'BestMakespan': best_makespan,
            'AvgMakespan': avg_makespan,
            'TrainingTime': training_time
        })

    def save(self):
        """Guarda todas las métricas acumuladas en el archivo CSV"""
        with open(self.filename, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            for metric in self.metrics:
                writer.writerow([
                    metric['Episode'],
                    metric['CurrentMakespan'],
                    metric['BestMakespan'],
                    metric['AvgMakespan'],
                    metric['TrainingTime']
                ])
        self.metrics = []  # Limpiar métricas después de guardar
...
class PPOAgent:
    ...
    def plot_best_solution_makespan(self):
        ...
        plt.figure(figsize=(12, 6))
        plt.plot(self.best_makespan_history, '-o')
        plt.axhline(y=930, color='r', linestyle='--', label='Óptimo: 930')
        plt.xlabel('Pasos')
        plt.ylabel('Makespan')
        plt.title('Evolución del Makespan de la Mejor Solución')
        plt.legend()
        plt.grid(True)
...
def main():
    ...
    csv_logger = TrainingLogger(filename="training_metrics.csv")
