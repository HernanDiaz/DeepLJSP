...
Figure~\ref{fig:arch} summarizes the decision loop. At every step the
policy observes the partial schedule and the set of eligible
operations---the next unscheduled operation of each unfinished job.
Each candidate is described by $16$ dimensionless quantities and the
overall shop state by $12$ more
(Section~\ref{sec:features}). A shared network embeds every candidate
independently; the embeddings are pooled into a single context vector
that summarizes the decision as a whole; and a scoring head rates each
candidate against that context, yielding a probability distribution
over the eligible operations (Section~\ref{sec:arch}). The sampled or
argmax choice is scheduled at its earliest feasible interval start
(Eq.~\eqref{eq:semiactive}), and the loop repeats until all $nm$
operations are placed. Training adjusts the shared weights with PPO
from episode rewards (Section~\ref{sec:training}); because no weight
dimension depends on $n$ or $m$, the same trained network dispatches
...
and $g_0\in\mathbb{R}^{12}$ the global features (Figure~\ref{fig:arch}).
