...
\title[Online Resource 1]{Online Resource 1 for: Deep Reinforcement
...
\abstract{This resource contains material referenced from the paper:
the automatic hyperparameter-configuration campaigns and their search
spaces (Section~\ref{sup:config}), the architecture and controlled
ablation of the self-attention variant (Section~\ref{sup:attention}),
and the per-instance results on the 70-instance benchmark
(Section~\ref{sup:perinstance}). Section, table and equation numbers
of the form S$n$ refer to this document; all other references are to
the paper.}
