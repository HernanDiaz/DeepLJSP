# -*- coding: utf-8 -*-
"""Amplia el deposito de la curva de presupuesto a diez tiradas.

Mismo protocolo que eval_budget_curve.py: por (tirada, instancia), 342
rollouts (el 0 greedy, el resto muestreados) con el E[Cmax]
componentwise de cada uno. Cubre las semillas 5-11 del brazo principal
(campana ext-c); las 2-4 ya estan en benchmarks/eval_budget_curve.csv,
que no se toca. Cada carril escribe su fichero, reanudable.

    python scripts/eval_curva_diez.py --carril 0 --de 6

Salida NUEVA: benchmarks/curva_diez/curva_<carril>.csv
"""
import argparse
import csv
import glob
...
def main():
    ...
    args = ap.parse_args()
    salida = f"benchmarks/curva_diez/curva_{args.carril}.csv"
    ...
    for s, inst in unidades:
        ...
        t0 = time.time()
        for i, mid in enumerate(rollouts(ckpt, inst, N_MUESTRAS)):
            w.writerow([etiqueta, inst, lb, i, f"{mid:.1f}"])
        f.flush()
